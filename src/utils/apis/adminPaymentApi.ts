...
): Promise<ApiPaginatedResponse<AdminfetchAllPaymentsResponse>> => {
...
export const updatePayment = async (
  paymentId: string,
  paymentData: UpdatePaymentFormData,
) => {
...
export const getPaymentsByCustomer = async (
  ...
  const query = buildQueryParams(params);
  const response = await axiosInstance.get(
    `/admin/payments/customer/${customerId}${query ? `?${query}` : ""}`,
  );
  return parseNewCommonResponse<AdminfetchAllPaymentsResponse>(response.data);
};

// Get payments by package
export const getPaymentsByPackage = async (
  packageId: string,
  params?: FetchFunctionParams,
): Promise<ApiPaginatedResponse<AdminfetchAllPaymentsResponse>> => {
  const query = buildQueryParams(params);
  const response = await axiosInstance.get(
    `/admin/payments/package/${packageId}${query ? `?${query}` : ""}`,
  );
  return parseNewCommonResponse<AdminfetchAllPaymentsResponse>(response.data);
};
...
export const getPaymentsByStatus = async (
  ...
  const query = buildQueryParams(params);
  const response = await axiosInstance.get(
    `/admin/payments/status/${status}${query ? `?${query}` : ""}`,
  );
  return parseNewCommonResponse<AdminfetchAllPaymentsResponse>(response.data);
};
