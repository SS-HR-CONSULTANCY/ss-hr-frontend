...
export const connectChatSocket = createAsyncThunk<
  ...
  void,
  { state: RootState }
>("chat/connectSocket", async (_, { getState, dispatch }) => {
...
export const userFetchUsersFroChatSideBar =
  async (): Promise<FetchAllUsersForChatSidebarResponse> => {
    const response = await axiosInstance.get("/user/chat/admins");
    return response.data.data;
  };

export const adminFetchUsersFroChatSideBar =
  async (): Promise<FetchAllUsersForChatSidebarResponse> => {
