...
import type {
  setLatMessageProps,
  UserProps,
} from "@/types/componentTypes/chatTypes";
import {
  setLastMessage,
  setOnlineUsers,
  setSelectedUser,
...
const ChatSidebar: React.FC<ChatSideBarProps> = ({ getUsers }) => {
  ...
  const { selectedUser, lastMessages, onlineUsers } = useSelector(
  ...
  return (
    <aside
      className={`overflow-y-scroll h-full w-full md:w-4/12 flex flex-col transition-all duration-200 sticky border-r-2 no-scrollbar ${selectedUser ? "hidden md:block" : "block"}`}
    >
