...
const UserAddressSection: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<AddressForm> = async (data) => {
    ...
    try {
      await dispatch(
        createAddress({
          id: update ? userAddress?._id : null,
          data: cleanedData as Address,
          update,
        }),
      )
        .unwrap()
        .then((res) => {
          if (res.success) {
            toast.success(
              res.message ||
                `Address ${update ? "updated" : "created"} successfully!`,
            );
            setIsEditing((prev) => !prev);
          } else {
            toast.error(
              res.message ||
                `Address ${update ? "updating" : "creating"} failed!`,
            );
          }
        })
        .catch((error) => {
          toast.error(
            error.message || `Address ${update ? "update" : "create"} error`,
          );
        });
    } catch {
  ...
  return (
    <div className="p-4 md:p-6 rounded-md border mt-4 shadow-md">
      <div className="flex justify-between items-center">
        <h3 className="text-lg md:text-2xl font-semibold my-2">Address</h3>
        {userAddress && (
          <Button
            variant={"outline"}
                ...
                <div className="flex items-center space-x-3 mt-4">
                  <Toggle
                    ...
                    className={`text-sm font-medium ${
                      field.value ? "bg-primary text-white" : "bg-muted"
                    }`}
                  >
                    <Check
