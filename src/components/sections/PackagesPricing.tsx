  ...
  return (
    <section id="packages" className="py-16">
      <div className="max-w-7xl mx-auto px-4 md:px-0">
        ...
        />
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 py-6`}
        >
          {packages.map((item) => (
            <Card
              key={item.name}
              className={`p-4 rounded-2xl shadow-sm flex flex-col border-2 hover:border-[#01487e] ${item.popular && "border-2 border-[#01487e]"}`}
              data-aos="fade-down"
            >
              ...
              {isAuthenticated ? (
                ...
                <CardFooter>
                  <Button
                    className="w-full border-0 shadow-md hover:font-bold text-white hover:text-white cursor-pointer bg-gradient-to-r to-[#ffd93e] from-[#01487e]"
                    variant={"outline"}
                  >
                    Sign up
                  </Button>
