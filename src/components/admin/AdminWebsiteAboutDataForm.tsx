...
const AdminWebsiteAboutDataForm: React.FC = () => {
    const [currentData, setCurrentData] = useState<AdminGetAboutCurrentDataResponse | null>(null);
    ...
    const onSubmitAbout: SubmitHandler<AboutSettingsFormValues> = async (data) => {
        try {
    ...
    return (
        <section className="shadow-md p-4 bg-gradient-to-r from-slate-50 to-sky-50 dark:from-slate-800 dark:to-black rounded-md">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                {/* Left: Current Data */}
                <form onSubmit={handleSubmitAbout(onSubmitAbout)} className="md:w-1/2 space-y-4 p-4  rounded shadow">
                    <FormField
                        id="aboutText"
                        label="About Text"
                    ...
                    />
                    <Button variant="outline" type="submit">Save About Settings</Button>
                </form>
