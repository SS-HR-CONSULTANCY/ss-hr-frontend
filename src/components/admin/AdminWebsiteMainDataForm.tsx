...
const AdminWebsiteMainDataForm: React.FC = () => {
    const [currentData, setCurrentData] = useState<AdminGetWebsiteSettingsResponse | null>(null);
    ...
    const onSubmitWebsite: SubmitHandler<WebsiteSettingsFormValues> = async (data) => {
        ...
        <section className="shadow-md p-4 bg-gradient-to-r from-slate-50 to-sky-50 dark:from-slate-800 dark:to-black rounded-md">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-1/2 space-y-4 p-4 rounded shadow">
                    ...
                    {currentData ? (
                        <>
                            <p><strong>Company Name:</strong> {currentData.companyName}</p>
                            {currentData.logo && (
                                <div>
                                    <strong>Logo:</strong>
                                    <img src={currentData.logo} alt="Current Logo" className="size-24 object-contain mt-2 border" />
                                </div>
                            )}
                        </>
                    ) : (
